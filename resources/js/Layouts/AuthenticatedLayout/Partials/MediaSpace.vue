...
<template>
    ...
    >
        <div id="media-space-container" class="transform-none">
            <MediaSpaceImage
                v-if="
                    mediaStore.type === 'App\\Models\\Image' ||
                    mediaStore.type === 'App\\Models\\Manga'
                "
                class="media-element"
                :style="
                    isMediaNarrower
                        ? `height: ${screenHeight - 1}px`
                        : `width: ${screenWidth - 1}px`
                "
            />
            <MediaSpaceAudio
                v-else-if="mediaStore.type === 'App\\Models\\Audio'"
                class="media-element"
            />
            <MediaSpaceVideo
                v-else-if="mediaStore.type === 'App\\Models\\Video'"
                class="media-element"
                :style="
                    isMediaNarrower
                        ? `height: ${screenHeight - 1}px`
        ...
        >
            <i-mingcute-close-fill
                class="h-[6dvh] w-[6dvw] focus:outline-none"
            />
        </button>
        <!--次・前のメディアへ移動する矢印-->
        <button
            @click="setPreviousMedia"
        ...
        <!--左上のハンバーガー-->
        <div
            class="fixed top-[2dvh] left-[2dvw] transition-opacity duration-300"
            :class="{
                'opacity-0': !isControlsVisible,
                'opacity-100': isControlsVisible,
            }"
        ...
        >
            <button
                @click="() => (isMenuBarVisible = !isMenuBarVisible)"
                class="rounded text-white/50"
            >
                <i-mingcute-menu-fill class="h-[4dvh] w-fit" />
            </button>
            <ul
                v-if="isMenuBarVisible"
                class="bg-white/50 py-8 rounded-lg [&>li]:flex [&>li]:items-center [&>li]:justify-between [&>li]:py-8 [&>li]:px-4 [&>li]:cursor-pointer"
            >
                <li
                    v-if="mediaStore.type === 'App\\Models\\Video'"
