...
export const useMediaStore = defineStore("media2", () => {
    ...
    const setMediaData = (media, page) => {
        ...
        preview_image_path.value = getPrivateStoragePath(
            media.preview_image_path,
        );
        ...
        switch (type.value) {
            case "App\\Models\\Audio":
                ...
                raw_image_path.value = getPrivateStoragePath(
                    media.mediable.raw_image_path,
                );
                duration.value = media.mediable.duration;
                waveform_path.value = getPrivateStoragePath(
                    media.mediable.waveform_path,
                );
                if (
                    mediaElement.value &&
                    mediaElement.value instanceof HTMLAudioElement
                ) {
            ...
            case "App\\Models\\Manga":
                ...
                src.value = getPrivateStoragePath(
                    media.mediable.pages[page].path,
                );
                srcLite.value = getPrivateStoragePath(
                    media.mediable.pages[page].lite_path,
                );
                break;
            case "App\\Models\\Video":
                ...
                if (
                    mediaElement.value &&
                    mediaElement.value instanceof HTMLVideoElement
                ) {
                    mediaElement.value.src = src.value;
                    mediaElement.value.currentTime = 0;
                }
    ...
    const setMediaElement = (element) => {
        ...
        mediaElement.value.addEventListener("loadedmetadata", () =>
        ...
        mediaElement.value.addEventListener("volumechange", () =>
    ...
    const unsetMediaElement = () => {
        ...
        mediaElement.value.removeEventListener("loadedmetadata", () =>
            handleLoadedMetadata(),
        );
        mediaElement.value.removeEventListener("timeupdate", () =>
            handleTimeupdate(),
        );
        mediaElement.value.removeEventListener("play", () => handleOnplay());
        mediaElement.value.removeEventListener("pause", () => handlePause());
        mediaElement.value.removeEventListener("volumechange", () =>
            handleOnvolumechange(),
        );
