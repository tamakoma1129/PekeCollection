...
<template>
    <div class="flex items-center w-full h-40 gap-x-16 relative"
         :class="{ 'bg-blue-200': isSelected }"
         @click="toggleSelection">
        <div>
            <!-- Audioのアイコン -->
            <span v-if="mediaFile.mediable_type === 'App\\Models\\Audio'">
                    <i-fa6-solid-headphones class="w-16 h-16 box-content bg-teto-500 p-4 text-white" />
            </span>
            <!-- Videoのアイコン -->
            <span v-else-if="mediaFile.mediable_type === 'App\\Models\\Video'" class="flex group-hover:bg-black h-full w-full items-center justify-center overflow-hidden">
                    ...
                    <i-pepicons-pop-clapperboard class="w-16 h-16 box-content bg-blue-500 p-4 text-white" />
            </span>
            <!-- Imageのアイコン -->
            <span v-else-if="mediaFile.mediable_type === 'App\\Models\\Image'" class="flex h-full w-full items-center justify-center overflow-hidden">
                    <i-pepicons-pop-photo class="w-16 h-16 box-content bg-green-500 p-4 text-white" />
            </span>
            <!-- Mangaのアイコン -->
            <span v-else-if="mediaFile.mediable_type === 'App\\Models\\Manga'" class="flex h-full w-full items-center justify-center overflow-hidden">
                    <i-bi-book class="w-16 h-16 box-content bg-violet-500 p-4 text-white" />
            </span>
        </div>
        <!-- ホバー時 -->
        ...
        <div class="w-1/4 select-none">
            <input type="text"
                   ...
                   class="w-auto min-w-104 max-w-full bg-transparent border-transparent py-4 overflow-x-auto select-none border-b border-b-sumi-300
                   focus:outline-none focus:ring-0 focus:border-x-0 focus:border-t-0 focus:border-b focus:border-b-sumi-500"
        ...
        <div class="min-w-104 overflow-x-auto">
            <ul class="flex gap-8">
                <li v-for="tag in mediaFile.tags"
                    :key="tag.name"
