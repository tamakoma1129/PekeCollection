...
<template>
    <dialog
        ref="deleteModal"
        class="rounded-xl backdrop:bg-sumi-800/50 bg-white w-2/3 h-5/6 overflow-y-auto"
    >
        <div class="h-full w-full" v-click-outside="closeDeleteModal">
            <h2 class="text-center text-2xl mt-16">
                以下のメディアを削除しますか？
            </h2>
            <div class="flex flex-wrap px-8 gap-8">
                <div
                    v-for="mediaFile in selectedDeleteMedia"
                    :key="mediaFile.id"
                    class="border border-sumi-300 w-[184px]"
                >
                    <div
            ...
            <div class="flex items-end justify-center gap-16 mt-16 pb-40">
                <div>
                    <p v-if="form.errors.password" class="text-teto-500">
                ...
                <danger-button @click="deleteMedia"> 削除する </danger-button>
            </div>
        </div>
    </dialog>
</template>
