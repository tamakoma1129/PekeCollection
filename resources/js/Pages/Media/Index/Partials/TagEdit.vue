...
<template>
    <div class="my-16 bg-white rounded-xl cursor-pointer" @click.self="toggleOpen">
        <button @click="toggleOpen" class="box-content flex items-center pl-4 pr-16">
            ...
            <span class="font-semibold" :class="{'text-base': !isOpen, 'text-lg': isOpen}">タグの編集</span>
        </button>
        <div v-if="isOpen" class="px-16 pb-8 cursor-default">
            ...
            <div class="mt-8 px-16 border border-sumi-400 rounded-xl">
                <div class="flex justify-between items-center pb-8">
                    ...
                    <tool-tip message="タグ情報を更新する">
                        <button @click="updateTags"><i-tabler-reload class="w-24 h-24" /></button>
                    </tool-tip>
                </div>
                <ul class="flex flex-wrap gap-4 pb-16">
                    <li v-for="tag in top30Tags"
                        :key="tag.name"
            ...
            <!-- 一時タグ置き場 -->
            <div class="mt-8">
                <button v-if="!isViewTempTags"  @click="toggleViewTempTags" class="box-content border border-sumi-400 rounded-full flex pl-8 pr-16 gap-8">
                    <i-icon-park-outline-up class="h-24 w-24"/>
                    <span>一時タグ置き場</span>
                </button>
                <div v-if="isViewTempTags"
                    ...
                    <ul class="flex flex-wrap gap-4 pb-16 pl-8">
                        <li v-for="tag in tempTags"
                            :key="tag"
                            class="flex items-center divide-sumi-400 divide-x w-fit"
                            draggable="true"
                            @dragstart="startDrag(tag)">
                            <span class="px-8 bg-sumi-200 rounded-l-xl h-24">{{ tag }}</span>
                            <button class="px-4 bg-sumi-200 hover:bg-teto-400 rounded-r-xl h-24" @click="removeTempTag(tag)">
            ...
            <!-- 選択されているタグ -->
            <div class="px-16 border border-sumi-400 rounded-xl mt-8"
                 @dragover.prevent
                 @drop="onDropInSelect" >
                ...
                <ul class="flex gap-4">
                    <li v-for="selectedTag in selectedTags" :key="selectedTag" class="flex items-center divide-sumi-400 divide-x w-fit"
                        draggable="true"
                        @dragstart="startDrag(selectedTag)">
                        <span class="px-8 bg-sumi-200 rounded-l-xl h-24">{{ selectedTag }}</span>
                        <button class="px-4 bg-sumi-200 hover:bg-teto-400 rounded-r-xl h-24" @click="unselectTag(selectedTag)">
                            <i-mdi-close class="w-full h-auto" />
                        </button>
                    </li>
                </ul>
            ...
            <div class="flex items-center justify-center mt-8">
                <p>選択されているタグ を 選択中の {{ mediaEditStore.selectedMediaIds.length }} 個のメディア</p>
                <div class="flex flex-col gap-8">
                    <div class="flex items-center">
                        <button @click="attachTags" class="w-fit py-4 px-8 bg-miku-500 hover:bg-miku-600 text-white rounded-xl">
                            に適用
                        </button>
                    </div>
                    <div class="flex items-center">
                        <button @click="detachTags" class="w-fit py-4 px-8 bg-teto-500 hover:bg-teto-600 text-white rounded-xl">
                            から削除
                        </button>
                    </div>
                </div>
