...
<template>
    ...
    >
        <p class="select-none">
            {{ mediaEditStore.selectedMediaIds.length }} /
            {{ mediaList.mediaList.length }} 選択中
        </p>
        <ToolTip message="全ての選択を解除" class="ml-16">
            <button
                class="flex items-center rounded-full p-4"
                :class="
                    mediaEditStore.selectedMediaIds.length === 0
                        ? 'text-sumi-400'
                        : 'hover:bg-sumi-200'
                "
        ...
        <div class="flex items-center ml-8 relative">
            <button
                @click="isOpenActionMenu = !isOpenActionMenu"
            ...
            <div
                class="absolute bottom-40 bg-white/75 flex flex-col items-center rounded-xl shadow-black shadow-[0px_0px_12px] py-16"
